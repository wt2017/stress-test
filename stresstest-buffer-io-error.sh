...
run_error_device_io_test() {
    ...
    if [ -z "${ERROR_DEVICE:-}" ]; then
        print_warning "Error device not set, skipping error device IO test"
        return 1
    fi
    
    # Check if error device is mounted as filesystem or available as raw block device
    if [ "${ERROR_DEVICE_MOUNTED:-false}" = "true" ] && [ -n "${ERROR_TEST_DIR:-}" ]; then
        # Use mounted filesystem
        print_info "Using mounted error device filesystem at ${ERROR_TEST_DIR}"
        
        local test_file="${ERROR_TEST_DIR}/error_test_file"
        
        # Write test pattern to file
        dd if=/dev/urandom of="$test_file" bs=1M count=10 2>/dev/null || {
            print_warning "Failed to create test file on error device filesystem"
            return 1
        }
        
        # Run fio test on the file
        ...
        # Use raw block device directly - this is more likely to trigger blk_update_request errors
        print_info "Using raw block device ${ERROR_DEVICE} for direct access"
        
        # Run fio directly on the block device
        # Target sectors 1000-1099 (error sectors)
        # Convert sector offset to bytes: 1000 sectors * 512 bytes/sector = 512000 bytes
        # Size: 100 sectors * 512 bytes/sector = 51200 bytes
        $FIO_CMD --name=error_device_test --filename="${ERROR_DEVICE}" --ioengine=posixaio --direct=1 \
            --bs=512 --iodepth=1 --size=51200 --offset=512000 --rw=randrw --rwmixread=50 \
            --numjobs=1 --runtime="$((RUN_TIME / 2))" --time_based --group_reporting \
            --output="$LOG_DIR/error_device_fio.log" --output-format=json &
        ERROR_FIO_PID=$!
        
        print_info "Direct block device access to error sectors 1000-1099 (512000-563199 bytes)"
    fi
    ...
    print_info "Direct block device access is more likely to trigger blk_update_request: IO error messages"
...
run_all_tests_parallel() {
    ...
    if [ "$error_device_available" = "true" ]; then
        if [ -n "${ERROR_TEST_DIR:-}" ]; then
            print_info "Using error device for IO tests at ${ERROR_TEST_DIR}"
        else
            print_info "Using raw error device ${ERROR_DEVICE} (no filesystem)"
        fi
